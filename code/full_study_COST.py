...
# CORRECTED 2026-08-13, close-out session. Two things were wrong here and both
# were invisible to every gate.
#
# FIRST, the row this is read from is labelled, in the committed file itself,
# "MULTIPLIER: nominal x this = base-year 2026$. Do NOT divide." Section 2/3
# below divided by it. That inverts the series: a dollar spent in 2015 came out
# SMALLER in 2026 dollars than the nominal figure rather than larger, by a
# factor of about two at the start of the window. The same script's `study`
# object was meanwhile using the same dictionary the right way round, through
# the template's real_repurchase_price(), so one file held two conventions for
# one quantity and they disagreed by a factor of two. That is the duplication
# defect this repository keeps meeting, in its purest form, and it is exactly
# what pulling the entry effect into the template removes.
#
# SECOND, the year was lagged - `DEFL_SRC.get(y - 1)` - with no reason recorded
# anywhere. The deflator convention is calendar-year CPI-U keyed to the fiscal
# year, unlagged, which is what the Apple study uses and what the template's
# real_repurchase_price(), program_flows() and round_trip() already assumed.
#
# EVERY REAL FIGURE PRINTED BELOW MOVES AS A RESULT. Nothing published moves:
# the Costco document at docs/Costco-Buyback-Study-2026-08-12.docx was written
# by hand rather than generated by this script, is four addenda stale, and is
# not regenerated here.
DEFL = {y: DEFL_SRC.get(y, DEFL_SRC[max(DEFL_SRC)]) for y in range(2010, 2027)}
...
# PULLED INTO THE TEMPLATE 2026-08-13 (close-out session). The entry effect, its
# break-even rate, the entity-level abnormal earnings growth account and the
# earnings-timing decomposition were all written out here and, separately,
# written out again in code/gen_article.py for Apple. They now come from
# buyback_study_TEMPLATE.entry_effect(), which is the only definition.
#
# THE COSTCO NUMBERS BELOW MOVE, and the reason is the deflator correction
# recorded at the top of this file, not the relocation. Nothing published moves;
# the Costco document was written by hand, not generated here.
print("\n--- SECTION 2/3: real EPS, real price paid, forward earnings yield, entry effect ---")
EE = study.entry_effect(rho=COE_RATE)
real_eps = EE['real_eps']
real_price_paid = EE['real_price_paid']
entry_effect = {t: EE['per_year'][t] / 1000.0 for t in EE['tranches']}   # $bn
fwd_yield = {t: real_eps[t + 1] / real_price_paid[t] for t in EE['tranches']}
for t in EE['tranches']:
    print(f"FY{t}  real px paid {real_price_paid[t]:8.2f}  real EPS(t+1) {real_eps[t+1]:6.3f}  "
          f"fwd yield {100*fwd_yield[t]:5.2f}%  entry effect ${entry_effect[t]:+7.3f}bn")
for _y, _why in sorted(EE['excluded_years'].items()):
    print(f"FY{_y}  NO ENTRY EFFECT STRUCK: {_why}")

cum_entry = EE['total'] / 1000.0
...
_tranches = EE['tranches']
_band = EE['band']
_est = EE['estimators']
_prim = EE['primary']

print("\n--- EARNINGS-TIMING DECOMPOSITION (entry = price decision + earnings timing) ---")
print(f"  log-linear fitted real EPS trend: {100*EE['trend_growth']:+.2f}%/yr "
...
print(f"  identity residual across all six estimators: {EE['identity_residual']:.2e} (must be ~0)")

_dep = EE['timing_dependence']
...
real_ni = study.real_net_income()
real_div_distrib = study.real_distributions()
aeg_entity = EE['aeg_entity']
for y in sorted(aeg_entity):
...
# continuing effect + still-owed-per-share. The template computes the continuing
# effect against SHARES OUTSTANDING, which is the convention the published Apple
# study uses; this script previously used the weighted diluted count. The base
# is reported so the choice cannot be lost.
print("\n--- continuing effect & still owed per share ---")
print(f"(continuing effect divides by: {EE['per_share_base']})")
for t in _tranches:
    cont = EE['continuing'][t] / study.retired[t]
    entry_ps = EE['per_year'][t] / study.retired[t]
...
# A year whose operating/financial split is not determinable comes back None
# rather than crashing or being silently dropped (defect 13). Costco tags every
# input in every year, so nothing is skipped here - but the guard has to be
# written as though it might be, or the next company reintroduces the crash.
_split_yrs = [y for y in range(2016, 2026)
              if y in attrib and attrib[y]['operating'] is not None]
if len(_split_yrs) < len([y for y in range(2016, 2026) if y in attrib]):
    print("  NOTE: the operating/financial split is unavailable in "
          f"{[y for y in range(2016, 2026) if y in attrib and y not in _split_yrs]}")
sum_op = sum(attrib[y]['operating'] for y in _split_yrs)
sum_fin = sum(attrib[y]['financial'] for y in _split_yrs)
